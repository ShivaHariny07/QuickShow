...
const SeatLayout = () => {
  ...
  const getOccupiedSeats = async () => {
    ...
    try {
      ...
      if (data.success) {
        const seats = Array.isArray(data.occupiedSeats) ? data.occupiedSeats : [];
        setOccupiedSeats(seats);
        // Remove selected seats if now occupied
  ...
  const bookTickets = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        if (Array.isArray(data.unavailableSeats)) {
          toast.error(
            `Selected seats ${data.unavailableSeats.join(", ")} are not available.`
          );
          setSelectedSeats((prev) => prev.filter((seat) => !data.unavailableSeats.includes(seat)));
  ...
  const timingList = (date && effectiveDateTime[date]) || [];
      ...
      <div className="w-60 bg-primary/10 border border-primary/20 rounded-lg py-10 h-max md:sticky md:top-30">
        ...
        <div className="mt-5 space-y-1">
          {timingList.length > 0 ? (
            ...
            <p className="p-6 text-sm text-gray-400">No available timings for this date</p>
          )}
        </div>
      </div>
