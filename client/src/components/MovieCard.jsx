...
const MovieCard = ({movie}) => {
    ...
    // Add defensive check for movie object
    if (!movie) {
    ...
    <div className='flex flex-col justify-between p-3 bg-gray-800 rounded-2xl hover:-translate-y-1 transition duration-300 w-66'>
      ...
      <img onClick={()=> {navigate(`/movies/${movie._id}`); scrollTo(0, 0)}}
       ...
       <p className='text-sm text-gray-400 mt-2'>
        {movie.release_date ? new Date(movie.release_date).getFullYear() : 'Unknown'} • {movie.genres?.slice(0,2).map(genre => genre.name).join(" | ") || 'Unknown'} • {movie.runtime ? timeFormat(movie.runtime) : 'Unknown'}
       </p>
